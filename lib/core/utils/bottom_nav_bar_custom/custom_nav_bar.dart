...
import 'package:hr_app_flutter/core/utils/bottom_nav_bar_custom/indexed.dart';

enum Direction { forward, backward, stationary }

enum AnimationType { shrinkOutIn, snap, spinOutIn }

typedef CustomNavBarChildBuilder = Widget Function(
...
class CustomNavBar extends StatelessWidget {
  ...
  CustomNavBarChildBuilder? builder;
  ...
  CustomNavBar.iconData({
    ...
    this.animationType = AnimationType.snap,
    this.badges,
  ...
  // assert(iconText.length == iconData.length),
  // assert(activeBadgeColors?.length == 1 ||
  //     activeBadgeColors?.length == iconData.length),
  // assert(activeIconColors.length == 1 ||
  //     activeIconColors.length == iconData.length),
  // assert(badges?.length == iconData.length),
...
class _CustomNavBarInnerState extends State<_CustomNavBarInner>
  ...
  void _setActive(int newIndex) {
    ...
    if (widget.animationType == AnimationType.snap) {
  ...
  void _snapActiveIndicator(AnimationInfo info) {
    setState(() {
      indicatorX = targetX;
      indicatorColor = _getRollerColor(activeIndex, activeIndex, 1.0) as Color;
    });
    animationUpdate = AnimationUpdate(
      animationValue: 1,
    ...
    widget.onAnimate?.call(animationUpdate);
  }
  ...
  void _spinOutInActiveIndicator(AnimationInfo info,
    ...
    indicatorAnimationTween = Tween<double>(begin: 0, end: 1).animate(curve)
      ..addListener(() {
        ...
        indicatorColor =
            _getRollerColor(info.oldIndex, info.newIndex, curve.value) as Color;
  ...
  Color _getBadgeColor(int index) {
    if (index == activeIndex) {
      return widget.activeBadgeColors == null
          ? Colors.red
          : widget.activeBadgeColors!.length == 1
              ? Colors.red
  ...
  Widget _buildNavBarItem(Widget child, Indexed indexed) {
    ...
    return _NavBarItem(
      child,
      badge: widget.badges == null
          ? null
          : widget.badges!.length >= indexed.index + 1
