  ...
  Widget build(BuildContext context) => SliverList(
        delegate: SliverChildBuilderDelegate(
          (BuildContext context, int index) {
            ...
            final rightPadding = index == groupWidgets.length ? 25.0 : 8.0;
            return Padding(
            ...
            child: InkWell(
              ...
              onTap: () async {
                ...
                if (index == 0) {
                  ...
                  /// Маршрут на как получить больше?
...
class ElementHistoryOperationWidget extends StatelessWidget {
  const ElementHistoryOperationWidget({required this.item, super.key});
  final Transaction item;

  @override
  Widget build(BuildContext context) {
    final date = DateTime.fromMillisecondsSinceEpoch(item.createAt * 1000);
...
class _TralingHistoryWidgetState extends State<TralingHistoryWidget> {
  ...
  Widget build(BuildContext context) => Row(
        ...
        children: [
          if (widget.item.typeTtransaction == 1)
            Text(
              '-${widget.item.amount}',
              style: Theme.of(context)
                  .textTheme
...
class HistoryOperationCoinWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => const GroupedListViewHistoryOperation();
}
...
class GroupedListViewHistoryOperation extends StatelessWidget {
  ...
  bool compareDates(DateTime date1, DateTime date2) {
    ...
    final date1OnlyDate = DateTime(date1.year, date1.month, date1.day);
    final date2OnlyDate = DateTime(date2.year, date2.month, date2.day);
        ...
        builder: (context, state) {
          if (state.data?.transactions != null) {
            ...
            state.data!.transactions?.forEach((item) {
              ...
              var dateString = DateFormat('d MMM EEE, y.').format(date);
              final isSameDay = compareDates(today, date);
              final isYesterday = compareYesterday(date);
              if (isSameDay) {
                dateString = 'Сегодня';
              } else if (isYesterday) {
                dateString = 'Вчера';
