...
final class NotificationRestClient implements INotificationRestClient {
  ...
  Future<String> getCountNotifications() async {
    final response = await _restClient.get('auth/count_notifications');

    if (response case {'result': final data as Map<String, dynamic>}) {
      return (data['count'] as int).toString();
    }
    throw Exception('Error send Notifications');
  }
}
