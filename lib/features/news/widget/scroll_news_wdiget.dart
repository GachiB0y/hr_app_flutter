...
class _ScrollNewsWidgetState extends State<ScrollNewsWidget> {
  ...
  Widget build(BuildContext context) {
    const radius = 18.0;
    return BlocBuilder<EventEntityBloc, EventEntityState>(builder: (context, state) {
      if (state is EventEntityState$Processing) {
        ...
        return const SliverToBoxAdapter(child: Center(child: Text('Ошибка загрузки сервисов!')));
      } else {
        if (state.data == null) {
          return const SliverToBoxAdapter(child: Center(child: Text('Ничего не найденно!')));
        } else {
          return SliverList(
            delegate: SliverChildBuilderDelegate(
              (BuildContext context, int index) {
                EventEntity item = state.data!.listEventEntityLoaded.toList()[index];
                ...
                return GestureDetector(
                  ...
                  // },
                  child: Padding(
                    padding: EdgeInsets.only(right: 9.0, left: index == 0 ? 25 : 0),
                    child: SizedBox(
                      child: Column(
                                  ...
                                  children: [
                                    ...
                                    Padding(
                                      ...
                                      child: Container(
                                        ...
                                        child: Text(
                                          DateFormat('dd MMMM', 'ru').format(item.startDate!),
