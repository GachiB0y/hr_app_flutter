...
class ApprovementNewsBloc extends Bloc<ApprovementEvent, ApprovementNewsState> {
  ...
  ApprovementNewsBloc({
    ...
    on<ApprovementEvent>(
      (event, emit) => event.map<Future<void>>(
        ...
        moveInArchiveNews: (event) async => await _onMoveInArchiveNews(event, emit),
      ),
    );
  }

  Future<void> _onApprovedNews(ApprovementEventApprovedNews event, Emitter<ApprovementNewsState> emit) async {
    try {
  ...
  Future<void> _onMoveInArchiveNews(
      ApprovementEventMoveInArchiveNews event, Emitter<ApprovementNewsState> emit) async {
    try {
      await eventEntityRepository
          .moveInArchiveNews(
            id: event.id,
  ...
  Future<void> _onApprovementEventFetch(Emitter<ApprovementNewsState> emit) async {
